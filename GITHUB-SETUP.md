...
- After it is enabled and filled in, the app also uploads once per local day when it is open. A spinner shows it is saving, then a message says it was saved. Nothing uploads if the app is closed. Writing a line never starts an upload. Several tabs in the same browser share one daily upload.
...
1. On GitHub, open `DoingRightNow-backup.json`.
